...
export default function Cta() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // Split-text reveal on the heading, on top of the scrubbed scale.
      ...
      // Curtain: dark panel wipes up to reveal the CTA as it scrolls in.
      // Default CSS state is "revealed", so no-JS/reduced users never see it.
      if (panelRef.current) {
        gsap.fromTo(
          panelRef.current,
  ...
  return (
    <section ref={scope} id="contact" className="section-pad" style={{ background: "#0e0e0f" }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div
          className="relative rounded-3xl overflow-hidden"
          style={{ background: "#131314" }}
        >
          {/* Curtain panel (JS drives it; hidden by default via clip-path) */}
          <div
            ref={panelRef}
            aria-hidden="true"
            className="absolute inset-0 z-20 pointer-events-none"
            style={{ background: "#0a0a0b", clipPath: "inset(0% 0% 100% 0%)" }}
          />
          <div
          ...
          <div className="relative z-10 px-6 sm:px-8 py-14 sm:py-20 lg:px-20 text-center">
            ...
            <h2 ref={headingRef} className="display-m text-white mb-5 max-w-2xl mx-auto">
              Let&apos;s build your{" "}
